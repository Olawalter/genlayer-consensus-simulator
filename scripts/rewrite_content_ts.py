"""Rewrite lib/learn/content.ts cleanly using template literals for code/body fields."""
import pathlib

ROOT = pathlib.Path("C:/GenB/GenLayer Consensus Simulator")
dest = ROOT / "lib/learn/content.ts"

# Write the file with template literals for all multi-line strings
content = r'''// ─── Types ───────────────────────────────────────────────────────────────────
...
lines = content.split("\n")
print(f"Wrote content.ts — {len(lines)} lines, {len(content)} bytes.")
print("Using template literals for all code blocks — no apostrophe/newline issues.")
